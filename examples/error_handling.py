...
def simulate_unreliable_data_source():
    ...
    failure_type = random.choice(['success', 'incomplete', 'corrupted', 'timeout', 'success'])
    ...
    if failure_type == 'success':
        ...
        data = pd.DataFrame({
            'id': range(1, 101),
            'value': [None] * 50 + list(np.random.normal(100, 20, 50)),  # Half missing
            'category': ['INVALID'] * 30 + list(np.random.choice(['A', 'B', 'C'], 70)),
            'timestamp': datetime.now()
        })
...
def robust_data_processing():
    ...
    try:
        ...
        invalid_count = len(data_cleaned) - len(data_cleaned[data_cleaned['category'].isin(valid_categories)])
    ...
    error_report = {
        "frontmatter": {
            ...
            "processing_status": "completed_with_issues" if error_log else "completed_successfully",
