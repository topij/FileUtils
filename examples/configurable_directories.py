...
def demonstrate_customer_success_workflow(temp_dir):
    ...
    sales_df = file_utils.load_single_file(
        "cs_analysis_Sales.csv", input_type="processed"
    )
    sales_df["ai_insights"] = sales_df["amount"].apply(
        lambda x: "High Value" if x > 2000 else "Standard"
    )
    sales_df["recommendation"] = sales_df["customer"].apply(
        lambda x: "Upsell" if "Corp" in x else "Retain"
    )
...
def demonstrate_content_creation_workflow(temp_dir):
    ...
    processed_content["publish_date"] = pd.date_range(
        "2024-01-01", periods=len(processed_content), freq="W"
    )
...
def demonstrate_research_workflow(temp_dir):
    ...
    analysis_results["normalized"] = (
        analysis_results["measurement"] - analysis_results["measurement"].mean()
    ) / analysis_results["measurement"].std()
...
def demonstrate_backward_compatibility(temp_dir):
    ...
    # Test loading
    loaded_data = file_utils.load_single_file(
        "compatibility_test.csv", input_type="raw"
    )
