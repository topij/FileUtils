...
def simulate_ai_analysis():
    ...
    sentiments = np.random.choice(['positive', 'negative', 'neutral'], 100, p=[0.6, 0.2, 0.2])
    ...
    for customer in customers:
        ...
        recommended_products = np.random.choice(products, size=np.random.randint(2, 5), replace=False)
...
def run_ai_workflow():
    ...
    top_recommendations = recommendation_data.groupby('product')['recommendation_score'].mean().sort_values(ascending=False)
    ...
    insights_doc = {
        "frontmatter": {
            ...
            "tags": ["AI", "Sentiment Analysis", "Recommendations", "Customer Insights"]
        },
...
"""
    ...
    api_response = {
        ...
        "results": {
            ...
            "recommendations": {
                ...
                "average_score": float(recommendation_data['recommendation_score'].mean()),
                "top_products": top_recommendations.head(5).to_dict(),
                "high_confidence_count": int(len(recommendation_data[recommendation_data['recommendation_score'] > 0.8]))
            }
        },
        "metadata": {
    ...
    summary_report = {
        ...
        "quality_metrics": {
            "average_confidence": float(avg_confidence),
            "high_confidence_ratio": float(len(recommendation_data[recommendation_data['recommendation_score'] > 0.8]) / len(recommendation_data)),
