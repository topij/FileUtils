    ...
    for quarter in quarters:
        for product in products:
            for region in regions:
                ...
                seasonal_factor = 1 + 0.2 * np.sin(2 * np.pi * (quarters.index(quarter) + 1) / 4)
...
def run_multi_format_reports():
    ...
    # Overall summary
    summary = report_data.groupby('quarter').agg({
        'sales': 'sum',
        'units': 'sum',
        'revenue': 'sum',
        'margin': 'mean'
    ...
    # Product performance
    product_performance = report_data.groupby('product').agg({
        'sales': 'sum',
        'units': 'sum',
        'revenue': 'sum',
        'margin': 'mean'
    ...
    # Regional analysis
    regional_analysis = report_data.groupby('region').agg({
        'sales': 'sum',
        'units': 'sum',
        'revenue': 'sum',
        'margin': 'mean'
