...
class TestExcelCsvConversionIntegration:
    ...
    def test_complete_roundtrip_workflow(self, temp_dir):
        ...
        sales_data = pd.DataFrame({
            'sale_id': [1, 2, 3, 4, 5],
            'customer_name': ['Alice Corp', 'Bob Industries', 'Charlie Ltd', 'Diana Inc', 'Eve Systems'],
        ...
        customers_data = pd.DataFrame({
            'customer_name': ['Alice Corp', 'Bob Industries', 'Charlie Ltd', 'Diana Inc', 'Eve Systems'],
            'industry': ['Technology', 'Manufacturing', 'Finance', 'Healthcare', 'Technology'],
        ...
        file_utils.save_data_to_storage(
            ...
            output_type="processed",
            file_name=original_csv_filename.replace('.csv', ''),  # Remove .csv extension
        ...
        pd.testing.assert_frame_equal(customers_reconstructed, customers_data, check_dtype=False)
        pd.testing.assert_frame_equal(products_reconstructed, products_data, check_dtype=False)
        
        # Step 5: Verify data integrity
        assert sales_reconstructed['total_amount'].sum() == sales_data['total_amount'].sum()
        assert sales_reconstructed['profit_margin'].sum() == sales_data['total_amount'].sum() * 0.15
